...
class SlacEvseSession(SlacSession):
    ...
    async def cm_slac_match(self):
        ...
        while True:
            ...
            # Extract NMK hash from REQ (last 32 bytes)
            received_nmk_hash = data_rcvd[-32:]

            # Compute expected hash from our IBE-derived NMK
            expected_nmk_hash = sha256(self.nmk).digest()

            # Verify
            if received_nmk_hash == expected_nmk_hash:
                print(f"\n  ✓ NMK HASH VERIFICATION PASSED")
                #print(f"  ▶ EV   Hash : {received_nmk_hash.hex()}")
                #print(f"  ▶ EVSE Hash : {expected_nmk_hash.hex()}")
                #print(f"  ▶ Match     : CONFIRMED — both sides hold identical NMK\n")
            else:
                print(f"\n  ✗ NMK HASH VERIFICATION FAILED")
                #print(f"  ▶ EV   Hash : {received_nmk_hash.hex()}")
                #print(f"  ▶ EVSE Hash : {expected_nmk_hash.hex()}")
                raise ValueError("NMK hash mismatch — IBE key establishment failed")
        ...
        evse_nmk_hash = sha256(self.nmk).digest()
        frame_to_send = (
            ether_header.pack_big()
            + homeplug_header.pack_big()
        ...
        print(f"  ▶ NMK in CNF : bytes(16) zeros — real NMK never transmitted ✓")
        print()
