...
def ibeExchange(key_dist_raw, enc_raw):
    """
    IBE decryption on PEV side:
    1. Deserialize private key and master public params from key_dist_raw
    2. Deserialize ciphertext from enc_raw
    3. Decrypt to recover NMK element
    4. Send ACK back to EVSE
    """
    global eth_header, homeplug_header

    # Deserialize key distribution frame
    key_dist = IBEKeyDistReq_fromraw(key_dist_raw)
    pev_private_key  = pickle.loads(key_dist.private_key_bytes)
    master_public_key = pickle.loads(key_dist.params_bytes)

    # Deserialize and decrypt ciphertext
    enc = IBEEncReq_fromraw(enc_raw)
    ct  = pickle.loads(enc.ciphertext_bytes)

    group = PairingGroup('MNT224')
    ibe   = IBE_BonehFranklin(group)
    nmk_element = ibe.decrypt(master_public_key, pev_private_key, ct)

    # Derive NMK bytes same way EVSE did
    nmk_raw = sha256(str(nmk_element).encode()).digest()[:16]

    print("\n" + "="*60)
    print("IBE DECRYPTION (PEV side)")
    print("="*60)
    print(f"Decrypted NMK: {nmk_raw.hex()}")
    print("="*60 + "\n")

    # Send ACK
    homeplug_header.mm_type = CM_IBE_ACK | MMTYPE_RSP
    eth_header.src = PEV_MAC
    eth_header.dst = EVSE_MAC
    ack_payload = b'\x00'  # result = 0 (success)

    class AckPacket(Packet):
        name = "IBEAck"
        fields_desc = [XByteField("result", 0)]

    ack_frame = eth_header / homeplug_header / AckPacket()
    sendp(ack_frame, iface=IFACE, verbose=False)
    logger.debug("PEV: Sent IBE ACK")
    return nmk_raw
...
def run():
    ...
    ibeExchange(key_dist_raw, enc_raw)
    sleep(0.2)
