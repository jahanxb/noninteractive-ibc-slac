...
class SlacMatch(Packet):
    ...
    fields_desc = [
        ...
        XNBytesField("run_id", 0, 8),
        XNBytesField("rsvd", 0, 8),
        XNBytesField("nmk_hash", 0, 32),  # ← ADD THIS: SHA256 of NMK
    ]
...
def slacMatch():
    ...
    global eth_header, homeplug_header, host_mac_int, ev_nmk
    homeplug_header.mm_type = CM_SLAC_MATCH | MMTYPE_REQ
    evse_mac_int = int.from_bytes(bytes.fromhex(EVSE_MAC.replace(":", "")), "big")

    # Compute NMK hash — proof of possession without revealing NMK
    nmk_hash = sha256(ev_nmk).digest() if ev_nmk else bytes(32)
    nmk_hash_int = int.from_bytes(nmk_hash, "big")

    slac_match = SlacMatch(
        pev_mac=host_mac_int,
        evse_mac=evse_mac_int,
        nmk_hash=nmk_hash_int,   # ← include hash
    )
    frame_rsp = eth_header / homeplug_header / slac_match
    logger.debug("EV:9: Sending Slac Match Request")
    logger.debug(f"EV NMK Hash: {nmk_hash.hex()}")
    sendp(frame_rsp, iface=IFACE, verbose=False)
...
def run():
    ...
    cnf_raw = wait_for(CM_SLAC_MATCH | MMTYPE_CNF, timeout=20)
    if not cnf_raw:
        sniffer.stop()
    ...
    # ── NMK Hash Verification ────────────────────────────────
    evse_nmk_hash = cnf_raw[-32:]
    our_nmk_hash  = sha256(ev_nmk).digest()

    banner("NMK HASH VERIFICATION  (PEV Side)")
    if evse_nmk_hash == our_nmk_hash:
        print(f"  ✓ VERIFICATION PASSED")
        print(f"  ▶ PEV  Hash : {our_nmk_hash.hex()}")
        print(f"  ▶ EVSE Hash : {evse_nmk_hash.hex()}")
        print(f"  ▶ Match     : CONFIRMED — both sides hold identical NMK ✓")
        print(f"  ▶ Method    : SHA256(NMK) compared without exposing NMK")
    else:
        print(f"  ✗ VERIFICATION FAILED")
        print(f"  ▶ PEV  Hash : {our_nmk_hash.hex()}")
        print(f"  ▶ EVSE Hash : {evse_nmk_hash.hex()}")
        logger.error("NMK hash mismatch in CNF")
    banner_end()
