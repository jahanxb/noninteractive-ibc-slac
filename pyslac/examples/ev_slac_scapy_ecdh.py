...
def ecdhExchange():
    ...
    sendp(frame_rsp, iface=IFACE, verbose=False)
    logger.debug("EV:8: Sent ECDH Exchange Response")
...
def run():
    ...
    # Step 5 - wait for CM_ECDH_EXCHANGE.REQ
    logger.debug("Waiting for CM_ECDH_EXCHANGE.REQ...")
    req = wait_for(CM_ECDH_EXCHANGE | MMTYPE_REQ, timeout=20)
    if not req:
        sniffer.stop()
        logger.error("No CM_ECDH_EXCHANGE.REQ - aborting")
        return
    logger.debug("Received CM_ECDH_EXCHANGE.REQ")
    ecdhExchange()
    sleep(0.2)
