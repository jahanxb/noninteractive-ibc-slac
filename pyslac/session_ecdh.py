...
class SlacEvseSession(SlacSession):
    ...
    async def cm_ecdh_exchange(self):
        ...
        ethernet_header = EthernetHeader(
            dst_mac=self.pev_mac,   # fix: send to PEV not broadcast
        ...
        while True:
            try:
                ...
                # ── ECDH KEY EXCHANGE DISPLAY ──────────────────────────
                print("\n" + "="*60)
                print("ECDH KEY EXCHANGE")
                print("="*60)
                print(f"EVSE Public Key (sent):     {qc_bytes.hex()}")
                print(f"PEV  Public Key (received): {qv_bytes.hex()}")
                ...
                print(f"Shared Secret (ECDH):       {shared_secret.hex()}")
                print(f"Derived NMK (SHA256[:16]):  {self.nmk.hex()}")
                print("="*60 + "\n")
