...
class FramesSizes(int, Enum):
    ...
    * MessageSize = X bytes
    ...
    #CM_SLAC_MATCH_REQ = 117 # This is the hash size of the NMK that we are adding in the frame, so the size is 32 bytes bigger than the one defined in the standard, which is 85 bytes
    
    
    LINK_STATUS_CNF = 60


    CM_IBE_KEY_DIST = 512   # variable, large enough for IBE key
    CM_IBE_ENC      = 512   # variable, large enough for ciphertext
    CM_IBE_ACK      = 60    # small ack frame
...
CM_IBE_KEY_DIST  = 0x60A0  # IBE: EVSE sends PEV private key
CM_IBE_ENC       = 0x60A4  # IBE: EVSE sends encrypted NMK
CM_IBE_ACK       = 0x60A8  # IBE: PEV acknowledges decryption
...
MM_TYPE_NAMES = {
    ...
    CM_IBE_KEY_DIST | MMTYPE_REQ: "CM_IBE_KEY_DIST.REQ",
    CM_IBE_ENC      | MMTYPE_REQ: "CM_IBE_ENC.REQ",
    CM_IBE_ACK      | MMTYPE_RSP: "CM_IBE_ACK.RSP",
