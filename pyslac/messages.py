...
class IBEKeyDistReq:
    """
    IBE Key Distribution: EVSE sends PEV's IBE private key
    and master public parameters serialized as bytes
    |key_len (2 bytes)|private_key_bytes|params_len (2 bytes)|params_bytes|
    """
    private_key_bytes: bytes
    params_bytes: bytes

    def pack_big(self):
        key_len = len(self.private_key_bytes).to_bytes(2, 'big')
        params_len = len(self.params_bytes).to_bytes(2, 'big')
        return key_len + self.private_key_bytes + params_len + self.params_bytes

    @classmethod
    def from_bytes(cls, payload: bytes) -> "IBEKeyDistReq":
        offset = 19  # skip ethernet + homeplug headers
        key_len = int.from_bytes(payload[offset:offset+2], 'big')
        offset += 2
        private_key_bytes = payload[offset:offset+key_len]
        offset += key_len
        params_len = int.from_bytes(payload[offset:offset+2], 'big')
        offset += 2
        params_bytes = payload[offset:offset+params_len]
        return cls(private_key_bytes=private_key_bytes, params_bytes=params_bytes)


@dataclass
class IBEEncReq:
    """
    IBE Encrypted NMK: EVSE sends IBE-encrypted NMK to PEV
    |ct_len (2 bytes)|ciphertext_bytes|
    """
    ciphertext_bytes: bytes

    def pack_big(self):
        ct_len = len(self.ciphertext_bytes).to_bytes(2, 'big')
        return ct_len + self.ciphertext_bytes

    @classmethod
    def from_bytes(cls, payload: bytes) -> "IBEEncReq":
        offset = 19
        ct_len = int.from_bytes(payload[offset:offset+2], 'big')
        offset += 2
        ciphertext_bytes = payload[offset:offset+ct_len]
        return cls(ciphertext_bytes=ciphertext_bytes)


@dataclass
class IBEAckRsp:
    """
    IBE Acknowledgement: PEV confirms successful decryption
    |result (1 byte)| 0x00 = success
    """
    result: int = 0

    def pack_big(self):
        return self.result.to_bytes(1, 'big')

    @classmethod
    def from_bytes(cls, payload: bytes) -> "IBEAckRsp":
        offset = 19
