...
class SlacEvseSession(SlacSession):
    ...
    def __init__(self, evse_id: str, iface: str, config: Config):
        ...
        SlacSession.__init__(self, state=STATE_UNMATCHED, evse_mac=bytes.fromhex(host_mac.replace(":", "")))
    ...
    async def evse_slac_parm(self) -> None:
        ...
        print("="*65)
        print(f"  ▶ CM_SLAC_PARM.REQ received from EV")
        print(f"  ▶ CM_SLAC_PARM.CNF sent to EV")
        print()
        print(f"  ┌─────────────────────────────────────────────────┐")
        print(f"  │           IDENTITY DISCOVERY (EVSE Side)        │")
        print(f"  ├─────────────────────────────────────────────────┤")
        print(f"  │ EVSE learns PEV identity from frame source MAC  │")
        print(f"  │                                                  │")
        print(f"  │  PEV  MAC  : {self.pev_mac.hex(':'):<36} │")
        print(f"  │  IBE ID    : EV:{self.pev_mac.hex():<33} │")
        print(f"  │                                                  │")
        print(f"  │  EVSE MAC  : {self.evse_mac.hex(':'):<36} │")
        print(f"  │  IBE ID    : EVSE:{self.evse_mac.hex():<30} │")
        print(f"  │                                                  │")
        print(f"  │  Run ID    : {self.run_id.hex():<36} │")
        print(f"  │                                                  │")
    ...
    async def cm_ibe_key_establishment(self):
        ...
        print(f"[PKG] Master secret loaded from: {ibe.master_secret_path}")
        print(f"[EVSE] Private key extracted for identity: {evse_identity}")
        print(f"[EVSE] Private key (secret_key type): {type(evse_sk.secret_key)}")
        ...
        print("\n" + "="*60)
        print("IBE KEY ESTABLISHMENT (EVSE side)")
        print("="*60)
        print(f"EV   Identity: {ev_identity}")
        print(f"EVSE Identity: {evse_identity}")
        print(f"Run ID:        {self.run_id.hex()}")
        print(f"Derived NMK:   {self.nmk.hex()}")
        print("="*60 + "\n")
    ...
    async def atten_charac_routine(self):
        ...
        await self.cm_ibe_key_establishment()   # ← changed from cm_ecdh_exchange
